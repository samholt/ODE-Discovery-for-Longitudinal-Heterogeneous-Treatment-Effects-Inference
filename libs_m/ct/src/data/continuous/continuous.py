# -*- coding: utf-8 -*-
"""
[Treatment Effects with RNNs] Continuous version of the data simulation
...
def generate_params(num_patients, chemo_coeff, radio_coeff, window_size, lag, equation):
    """
    ...
    basic_params = get_standard_params(num_patients, equation=equation)
...
def get_standard_params(num_patients, equation: Equation):  # additional params
    """
    ...
    if equation.name == 'EQ_5_A' or equation.name == 'EQ_5_B':
        possible_patient_types = [1]
    elif equation.name == 'EQ_5_C' or equation.name == 'EQ_5_D':
    ...
    if equation.name == 'EQ_5_D':
        ...
        beta_c = beta_c_params[0] + beta_c_adjustments
...
def simulate_factual(simulation_params, seq_length, equation: Equation, assigned_actions=None):
    """
    ...
    if equation.name.split('_')[-1] in ['B', 'C', 'D']:
        cancer_volume = cancer_volume + observation_noise * np.random.normal(size=cancer_volume.shape)
    ...
    assert not np.any(np.isnan(outputs['cancer_volume'])), 'Cancer volume contains NaN'
...
def simulate_counterfactual_1_step(simulation_params, seq_length, equation: Equation):
    """
    ...
    chemo_dosage = np.zeros((num_test_points, seq_length))
    ...
    for i in tqdm(range(num_patients), total=num_patients):
        ...
        for t in range(0, seq_length - 1):
            ...
            chemo_dosage[test_idx] = factual_chemo_dosage
            ...
            for treatment_option in treatment_options:
                ...
                cancer_volume[test_idx][:t + 2] = np.append(factual_cancer_volume[:t + 1], [counterfactual_cancer_volume]) # Seems this is getting cut off?!
                chemo_dosage[test_idx][:t + 1] = np.append(factual_chemo_dosage[:t], [counterfactual_chemo_dosage])
                chemo_application_point[test_idx][:t + 1] = np.append(factual_chemo_application_point[:t], [counterfactual_chemo_application_point])
                radio_application_point[test_idx][:t + 1] = np.append(factual_radio_application_point[:t], [counterfactual_radio_application_point])
                patient_types_all_trajectories[test_idx] = patient_types[i]
                sequence_lengths[test_idx] = int(t) + 1
                test_idx = test_idx + 1
    ...
    outputs = {'cancer_volume': cancer_volume[:test_idx],
               'chemo_dosage': chemo_dosage[:test_idx],
...
def simulate_counterfactuals_treatment_seq(simulation_params, seq_length, projection_horizon, equation: Equation, cf_seq_mode='sliding_treatment'):
    """
    ...
    chemo_dosage = np.zeros((num_test_points, seq_length + projection_horizon))
    ...
    for i in tqdm(range(num_patients), total=num_patients):
        ...
        for t in range(0, seq_length - 1):
            ...
            for treatment_option in treatment_options:
                ...
                chemo_dosage[test_idx][:t + 1 + projection_horizon] = counterfactual_chemo_dosage
...
if __name__ == "__main__":
    ...
    for equation in Equation:
        if 'EQ_5' in equation.name:
            print('Now running: ', equation.name)

            params = generate_params(num_patients, chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, window_size=window_size, lag=lag, equation=equation)
            params['window_size'] = window_size
            training_data = simulate_factual(params, seq_length, equation=equation)

            params = generate_params(int(num_patients / 10), chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, window_size=window_size, lag=lag, equation=equation)
            params['window_size'] = window_size
            validation_data = simulate_factual(params, seq_length, equation=equation)

            params = generate_params(int(num_patients / 10), chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, window_size=window_size, lag=lag, equation=equation)
            params['window_size'] = window_size
            test_data_factuals = simulate_factual(params, seq_length, equation=equation)
            test_data_counterfactuals = simulate_counterfactual_1_step(params, seq_length, equation=equation)

            params = generate_params(int(num_patients / 10), chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, window_size=window_size, lag=lag, equation=equation)
            params['window_size'] = window_size
            test_data_seq = simulate_counterfactuals_treatment_seq(params, seq_length, 5, equation=equation)

            # Plot patient
            patient_number = int(num_patients * 0.572)
            plot_treatments(training_data, patient_number)
            plot_sigmoid_function({})
            print('')

            df = pd.DataFrame({'N(t)': training_data['cancer_volume'][patient_number],
                            'C(t)': training_data['chemo_dosage'][patient_number],
                            'd(t)': training_data['radio_dosage'][patient_number],
                            })
            df = df[['N(t)', "C(t)", "d(t)"]]
            df.plot(secondary_y=['C(t)', 'd(t)'])
            plt.xlabel("$t$")
            plt.show()
            plt.savefig(f'treatments_{patient_number}.png')