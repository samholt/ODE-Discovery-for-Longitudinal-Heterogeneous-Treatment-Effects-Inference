...
from src.data.continuous.continuous import TUMOUR_DEATH_THRESHOLD
from src.data.continuous.continuous import generate_params, get_scaling_params, simulate_factual, \
    simulate_counterfactual_1_step, simulate_counterfactuals_treatment_seq
from src.data.pkpd.pkpd_simulation import Equation
...
class SyntheticContinuousDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.params = generate_params(num_patients, chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, window_size=window_size,
                                      lag=lag, equation=equation)
        self.subset_name = subset_name

        if mode == 'factual':
            self.data = simulate_factual(self.params, seq_length, equation=equation)
        elif mode == 'counterfactual_one_step':
            self.data = simulate_counterfactual_1_step(self.params, seq_length, equation=equation)
        elif mode == 'counterfactual_treatment_seq':
            assert projection_horizon is not None
            self.data = simulate_counterfactuals_treatment_seq(self.params, seq_length, projection_horizon, equation=equation, cf_seq_mode=cf_seq_mode)
    ...
    def process_data(self, scaling_params, include_continuous_treatment=False):
        ...
        if not self.processed:
            ...
            if self.equation.name == 'EQ_5_A' or self.equation.name == 'EQ_5_B':
                std['patient_types'] = 1

            if include_continuous_treatment:
                input_means = mean[['cancer_volume', 'patient_types', 'chemo_dosage', 'chemo_application', 'radio_application']].values.flatten()
                input_stds = std[['cancer_volume', 'patient_types', 'chemo_dosage', 'chemo_application', 'radio_application']].values.flatten()
            else:
            ...
            if include_continuous_treatment:
                chemo_dosage = (self.data['chemo_dosage'] - mean['chemo_dosage']) / std['chemo_dosage']
            ...
            if include_continuous_treatment:
                current_covariates = np.concatenate([cancer_volume[:, :-offset, np.newaxis], patient_types[:, :-offset, np.newaxis], chemo_dosage[:, :-offset, np.newaxis]], axis=-1)
            else:
                current_covariates = np.concatenate([cancer_volume[:, :-offset, np.newaxis], patient_types[:, :-offset, np.newaxis]], axis=-1)
    ...
    def explode_trajectories(self, projection_horizon, include_continuous_treatment=False):
...
class SyntheticContinuousDatasetCollection(SyntheticDatasetCollection):
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            treatment_mode: multiclass / multilabel
        """
        super(SyntheticContinuousDatasetCollection, self).__init__()
        self.seed = seed
        equation = Equation[equation_str]
        self.equation = equation
        np.random.seed(seed)

        self.train_f = SyntheticContinuousDataset(chemo_coeff, radio_coeff, num_patients['train'], window_size, max_seq_length,
                                              'train', lag=lag, seed=seed, treatment_mode=treatment_mode, equation=equation)
        self.val_f = SyntheticContinuousDataset(chemo_coeff, radio_coeff, num_patients['val'], window_size, max_seq_length, 'val',
                                            lag=lag, seed=seed, treatment_mode=treatment_mode, equation=equation)
        self.test_cf_one_step = SyntheticContinuousDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size,
                                                       max_seq_length, 'test', mode='counterfactual_one_step', lag=lag, seed=seed,
                                                       treatment_mode=treatment_mode, equation=equation)
        self.test_cf_treatment_seq = SyntheticContinuousDataset(chemo_coeff, radio_coeff, num_patients['test'], window_size,
                                                            max_seq_length, 'test', mode='counterfactual_treatment_seq',
                                                            projection_horizon=projection_horizon, lag=lag, seed=seed,
