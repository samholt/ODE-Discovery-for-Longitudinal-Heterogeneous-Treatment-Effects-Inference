...
def calc_diameter(volume):
    diameter = ((volume / (4 / 3 * np.pi)) ** (1 / 3)) * 2
    if np.isnan(volume):
...
def generate_params(num_patients, chemo_coeff, radio_coeff, window_size, lag):
    ...
    :param window_size: Size of window for calculating, i.e., output mean window size for confounding treatment assignment
...
def simulate_factual(simulation_params, seq_length, assigned_actions=None):
    ...
    for i in tqdm(range(num_patients), total=num_patients):
        ...
        for t in range(1, seq_length - 1):
            ...
            # Not the valid equation that is actually presented in the nature paper! - need to add the value itself here and be an euler approximation
            if cancer_volume[i, t - 1] == 0:
                new_cancer_volume = 0
            else:    
                new_cancer_volume = cancer_volume[i, t - 1] * (1 + rho * protected_log(K / cancer_volume[i, t - 1]) - beta_c * chemo_dosage[i, t - 1] - (alpha * radio_dosage[i, t - 1] + beta * radio_dosage[i, t - 1] ** 2) + noise[t])
            new_cancer_volume = 0 if new_cancer_volume < 0 else new_cancer_volume
            cancer_volume[i, t] = new_cancer_volume
...
def simulate_counterfactual_1_step(simulation_params, seq_length):
    ...
    for i in tqdm(range(num_patients), total=num_patients):
        ...
        for t in range(0, seq_length - 1):
            ...
            # Factual prev_treatments and outcomes
            if factual_cancer_volume[t] == 0:
                new_factual_cancer_volume = 0
            else:    
                new_factual_cancer_volume = factual_cancer_volume[t] * \
                (1 + rho * protected_log(K / factual_cancer_volume[t]) - beta_c * factual_chemo_dosage[t] -
                    (alpha * factual_radio_dosage[t] + beta * factual_radio_dosage[t] ** 2) + noise[t + 1])
            new_factual_cancer_volume = 0 if new_factual_cancer_volume < 0 else new_factual_cancer_volume
            factual_cancer_volume[t + 1] = new_factual_cancer_volume
            ...
            for treatment_option in treatment_options:
                ...
                counterfactual_cancer_volume = factual_cancer_volume[t] *\
                    (1 + rho * protected_log(K / factual_cancer_volume[t]) - beta_c * counterfactual_chemo_dosage -
        ...
        for t in range(0, seq_length - 1):
            ...
            for treatment_option in treatment_options:
                ...
                for projection_time in range(0, projection_horizon):
                    ...
                    if counterfactual_cancer_volume[current_t] == 0:
                        new_counterfactual_cancer_volume = 0
                    else:    
                        new_counterfactual_cancer_volume = counterfactual_cancer_volume[current_t] *\
                        (1 + rho * protected_log(K / (counterfactual_cancer_volume[current_t] + 1e-07) + 1e-07) -
                         ...
                         noise[current_t + 1])
                    new_counterfactual_cancer_volume = 0 if new_counterfactual_cancer_volume < 0 else new_counterfactual_cancer_volume
                    counterfactual_cancer_volume[current_t + 1] = new_counterfactual_cancer_volume
...
def protected_log(x1):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        # return np.where(x1 > 1e-9, np.log(x1), 0.)
        return np.where(np.abs(x1) > 0.001, np.log(np.abs(x1)), 0.)
        # return np.where(np.abs(x1) > 1e6, np.log(np.abs(x1)), 0.)
