...
def generate_log_file_path(file, log_folder='logs', config = {}):
    ...
    return f"{log_folder}/{path_run_name}_{'-'.join(config.setup.ct_methods_to_evaluate)}_{'-'.join(config.setup.ct_datasets_to_evaluate)}_{config.setup.seed_start}_{config.setup.seed_runs}-runs_log.txt"
