...
from src.data.pkpd.pkpd_simulation import MAX_VALUE, Equation
from src.data.pkpd.pkpd_simulation import generate_params, get_scaling_params, simulate_factual, \
...
class SyntheticPkpdDataset(Dataset):
    """
    Pytorch-style Dataset of PKPD Simulator datasets
    """
    ...
    def __init__(self,
        ...
        """
        Args:
            conf_coeff: Confounding coefficient of action
        ...
        self.params = generate_params(num_patients, conf_coeff=conf_coeff, window_size=window_size, lag=lag, equation=equation, key=self.subkey)
        self.subset_name = subset_name

        if mode == 'factual':
            self.key, self.subkey = jax.random.split(self.key)
            self.data = simulate_factual(self.params, seq_length, equation=self.equation, key=self.subkey)
        elif mode == 'counterfactual_one_step':
            self.key, self.subkey = jax.random.split(self.key)
            self.data = simulate_counterfactual_1_step(self.params, seq_length, equation=self.equation, key=self.subkey)
        elif mode == 'counterfactual_treatment_seq':
            assert projection_horizon is not None
            self.key, self.subkey = jax.random.split(self.key)
            self.data = simulate_counterfactuals_treatment_seq(self.params, seq_length, projection_horizon, cf_seq_mode=cf_seq_mode, equation=self.equation, key=self.subkey)
    ...
    def process_data(self, scaling_params, include_continuous_treatment=False):
        """
        ...
        if not self.processed:
            ...
            if 'EQ_4' in self.equation.name:
                # Binary case
                mean['treatment_application'] = 0
                std['treatment_application'] = 1

            input_means = mean[['cancer_volume', 'observed_static_c_0', 'observed_static_c_1', 'treatment_application']].values.flatten()
            input_stds = std[['cancer_volume', 'observed_static_c_0', 'observed_static_c_1', 'treatment_application']].values.flatten()

            # Continuous values
            cancer_volume = (self.data['cancer_volume'] - mean['cancer_volume']) / std['cancer_volume']
            observed_static_c_0 = (self.data['observed_static_c_0'] - mean['observed_static_c_0']) / std['observed_static_c_0']
            observed_static_c_1 = (self.data['observed_static_c_1'] - mean['observed_static_c_1']) / std['observed_static_c_1']
            ...
            if self.treatment_mode == 'multiclass':
                ...
                for patient_id in range(treatments.shape[0]):
                    for timestep in range(treatments.shape[1]):
                        if (treatments[patient_id][timestep][0] == 0):
                            one_hot_treatments[patient_id][timestep] = [1, 0]
                        elif (treatments[patient_id][timestep][0] == 1):
            ...
            current_covariates = np.concatenate([cancer_volume[:, :-offset, np.newaxis], observed_static_c_0[:, :-offset, np.newaxis], observed_static_c_1[:, :-offset, np.newaxis]], axis=-1)
            ...
            for i in range(sequence_lengths.shape[0]):
                ...
                active_entries[i, :sequence_length, :] = 1 # Think depending how this is used; if masking should really be active_entries[i, :sequence_length + 1, :] = 1
    ...
    def process_sequential(self, encoder_r, projection_horizon, save_encoder_r=False,  include_continuous_treatment=False):
        """
    ...
    def process_sequential_test(self, projection_horizon, encoder_r=None, save_encoder_r=False,  include_continuous_treatment=False):
        """
        ...
        if not self.processed_sequential:
            ...
            seq2seq_data = {
                ...
                'active_entries': seq2seq_active_entries,
                'unscaled_outputs': seq2seq_outputs * self.scaling_params['output_stds'] + self.scaling_params['output_means'],
                'observed_static_c_0': self.data['observed_static_c_0'],
                'observed_static_c_1': self.data['observed_static_c_1'],
    ...
    def process_autoregressive_test(self, encoder_r, encoder_outputs, projection_horizon, save_encoder_r=False,  include_continuous_treatment=False):
        """
    ...
    def process_sequential_multi(self, projection_horizon, include_continuous_treatment=False):
        """
...
class SyntheticPkpdDatasetCollection(SyntheticDatasetCollection):
    """
    ...
    def __init__(self,
        ...
        """
        Args:
            conf_coeff: Confounding coefficient of treatment assignment
            num_patients: Number of patients in dataset
            window_size: Used for biased treatment assignment
            ...
            treatment_mode: multiclass / multilabel
        """
        super(SyntheticPkpdDatasetCollection, self).__init__()
        self.seed = seed
        equation = Equation[equation_str]
        self.equation = equation
        np.random.seed(seed)

        self.train_f = SyntheticPkpdDataset(conf_coeff, num_patients['train'], window_size, max_seq_length, 'train', lag=lag, seed=seed, treatment_mode=treatment_mode, equation=equation)
        self.val_f = SyntheticPkpdDataset(conf_coeff, num_patients['val'], window_size, max_seq_length, 'val', lag=lag, seed=seed, treatment_mode=treatment_mode, equation=equation)
        self.test_cf_one_step = SyntheticPkpdDataset(conf_coeff, num_patients['test'], window_size,
                                                       max_seq_length, 'test', mode='counterfactual_one_step', seed=seed, lag=lag,
                                                       treatment_mode=treatment_mode, equation=equation)
        self.test_cf_treatment_seq = SyntheticPkpdDataset(conf_coeff, num_patients['test'], window_size,
                                                            max_seq_length, 'test', mode='counterfactual_treatment_seq',
                                                            seed=seed,
                                                            projection_horizon=projection_horizon, lag=lag,
                                                            cf_seq_mode=cf_seq_mode, treatment_mode=treatment_mode, equation=equation)
